...
import { fetchTransactions, fetchSafeToSpend, deleteTransaction, type Transaction } from "@/lib/api";
...
export function SpendingsScreen({ currency = "USD", onClose }: Props) {
  ...
  const hasMonthlyBudget = summary?.monthlyBudget != null && summary.monthlyBudget > 0;
  const budgetLeft = hasMonthlyBudget ? (summary?.remainingInPeriod ?? 0) : null;
    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex flex-col">
      ...
      {hasMonthlyBudget && (
        <div className="mx-4 mt-4 p-4 rounded-xl bg-[var(--bg-secondary)] border border-[var(--xp-bar-bg)]">
          ...
          <p
            ...
            style={{
              color: (budgetLeft ?? 0) <= 0 ? "var(--xp-bar-danger)" : "var(--accent-green)",
            }}
          >
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <p className="text-[var(--text-secondary)] font-display">No spendings yet. Pull down on home to log one.</p>
        ) : (
                  ...
                  <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: tx.category?.color ? `${tx.category.color}30` : "var(--xp-bar-bg)" }}
                      />
                      <div>
                        ...
                        <p className="text-sm text-[var(--text-secondary)] font-display">
                          {formatDate(tx.date)} · {tx.spendType === "IMPULSIVE" ? "Impulsive" : "Planned"}
                        </p>
                      </div>
                    </div>
                    <p className="font-display font-bold text-[var(--text-primary)]">
