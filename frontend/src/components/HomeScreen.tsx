...
const MOCK_NUDGE = "You haven't logged anything today. Pull down when you spend so we can keep your Safe to Spend updated.";
...
export function HomeScreen({ onPullToAdd, onOpenBudget, onOpenSettings, onOpenSpendings, onOpenReflectionsMockup }: Props) {
  ...
  useDrag(
    ...
    { axis: "y", pointer: { touch: true }, target: ref, from: () => [0, y.get()] }
  );
    ...
    <div ref={ref} className="relative touch-none select-none">
      <motion.div style={{ y }} className="flex flex-col min-h-[80vh] px-4">
        ...
        <div className="flex flex-col items-center justify-center flex-1">
          ...
          {isLoading && !demoMode ? (
            <p className="text-display-hero font-display text-[var(--text-secondary)]">…</p>
          ) : (
            <p
          ...
          <div className="w-full max-w-xs mt-8 h-3 rounded-full bg-[var(--xp-bar-bg)] overflow-hidden">
            <motion.div
              ...
              style={{
                ...
                boxShadow: isDanger ? "0 0 12px var(--xp-bar-danger)" : undefined,
              }}
              initial={false}
              animate={{
                width: `${xpPercent}%`,
