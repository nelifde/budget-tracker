...
export async function fetchSafeToSpend(date?: string): Promise<SafeToSpendResult> {
  const url = date ? `${API_BASE}/api/summary/safe-to-spend?date=${encodeURIComponent(date)}` : `${API_BASE}/api/summary/safe-to-spend`;
...
export async function fetchSettings(): Promise<UserSettings> {
  const res = await fetch(`${API_BASE}/api/user/settings`, { headers: headers() });
  if (!res.ok) throw new Error(await res.text().catch(() => res.statusText));
  return res.json();
}
...
export async function fetchTransactions(params?: { from?: string; to?: string; limit?: number }): Promise<Transaction[]> {
  const sp = new URLSearchParams();
  if (params?.from) sp.set("from", params.from);
  if (params?.to) sp.set("to", params.to);
  if (params?.limit) sp.set("limit", String(params.limit));
  const q = sp.toString();
  const url = q ? `${API_BASE}/api/transactions?${q}` : `${API_BASE}/api/transactions`;
  const res = await fetch(url, { headers: headers() });
  if (!res.ok) throw new Error(await res.text().catch(() => res.statusText));
  return res.json();
}
...
export async function fetchBudgets(year?: number, month?: number): Promise<BudgetsResponse> {
  const sp = new URLSearchParams();
...
export async function createBudgetLine(name: string, amount: number): Promise<BudgetLine> {
